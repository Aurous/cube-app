export interface Theme {
  name: string
  colors: {
    bg: string
    bgSecondary: string
    main: string
    sub: string
    subAlt: string
    text: string
    error: string
    accent: string
    accentHover: string
    phaseCross: string
    phaseF2L1: string
    phaseF2L2: string
    phaseF2L3: string
    phaseF2L4: string
    phaseOLL: string
    phasePLL: string
    cubeWhite: string
    cubeYellow: string
    cubeGreen: string
    cubeBlue: string
    cubeRed: string
    cubeOrange: string
  }
}

export const themes: Record<string, Theme> = {
  kitsune: {
    name: 'Kitsune',
    colors: {
      bg: '#0c0a09',
      bgSecondary: '#1c1917',
      main: '#f97316',
      sub: '#78716c',
      subAlt: '#292524',
      text: '#fafaf9',
      error: '#ef4444',
      accent: '#f97316',
      accentHover: '#ea580c',
      phaseCross: '#c084fc',
      phaseF2L1: '#fb923c',
      phaseF2L2: '#fdba74',
      phaseF2L3: '#fed7aa',
      phaseF2L4: '#ffedd5',
      phaseOLL: '#fbbf24',
      phasePLL: '#4ade80',
      cubeWhite: '#fafaf9',
      cubeYellow: '#fbbf24',
      cubeGreen: '#22c55e',
      cubeBlue: '#3b82f6',
      cubeRed: '#ef4444',
      cubeOrange: '#f97316',
    },
  },
  dark: {
    name: 'Dark',
    colors: {
      bg: '#0a0a0a',
      bgSecondary: '#171717',
      main: '#e2b714',
      sub: '#646669',
      subAlt: '#2c2e31',
      text: '#d1d0c5',
      error: '#ca4754',
      accent: '#e2b714',
      accentHover: '#c9a312',
      phaseCross: '#a855f7',
      phaseF2L1: '#3b82f6',
      phaseF2L2: '#60a5fa',
      phaseF2L3: '#93c5fd',
      phaseF2L4: '#bfdbfe',
      phaseOLL: '#eab308',
      phasePLL: '#22c55e',
      cubeWhite: '#e0e0e0',
      cubeYellow: '#e2b714',
      cubeGreen: '#22c55e',
      cubeBlue: '#3b82f6',
      cubeRed: '#ef4444',
      cubeOrange: '#f97316',
    },
  },
  light: {
    name: 'Light',
    colors: {
      bg: '#f5f5f5',
      bgSecondary: '#e8e8e8',
      main: '#2c2e31',
      sub: '#8a8a8a',
      subAlt: '#d1d0c5',
      text: '#2c2e31',
      error: '#ca4754',
      accent: '#e2b714',
      accentHover: '#c9a312',
      phaseCross: '#9333ea',
      phaseF2L1: '#2563eb',
      phaseF2L2: '#3b82f6',
      phaseF2L3: '#60a5fa',
      phaseF2L4: '#93c5fd',
      phaseOLL: '#ca8a04',
      phasePLL: '#16a34a',
      cubeWhite: '#ffffff',
      cubeYellow: '#eab308',
      cubeGreen: '#16a34a',
      cubeBlue: '#2563eb',
      cubeRed: '#dc2626',
      cubeOrange: '#ea580c',
    },
  },
  serikaDark: {
    name: 'Serika Dark',
    colors: {
      bg: '#323437',
      bgSecondary: '#2c2e31',
      main: '#e2b714',
      sub: '#646669',
      subAlt: '#2c2e31',
      text: '#d1d0c5',
      error: '#ca4754',
      accent: '#e2b714',
      accentHover: '#c9a312',
      phaseCross: '#a855f7',
      phaseF2L1: '#3b82f6',
      phaseF2L2: '#60a5fa',
      phaseF2L3: '#93c5fd',
      phaseF2L4: '#bfdbfe',
      phaseOLL: '#eab308',
      phasePLL: '#22c55e',
      cubeWhite: '#d1d0c5',
      cubeYellow: '#e2b714',
      cubeGreen: '#22c55e',
      cubeBlue: '#3b82f6',
      cubeRed: '#ef4444',
      cubeOrange: '#f97316',
    },
  },
  serikaLight: {
    name: 'Serika Light',
    colors: {
      bg: '#e1e1e3',
      bgSecondary: '#d1d0c5',
      main: '#323437',
      sub: '#5f6368',
      subAlt: '#c5c5c5',
      text: '#323437',
      error: '#ca4754',
      accent: '#e2b714',
      accentHover: '#c9a312',
      phaseCross: '#9333ea',
      phaseF2L1: '#2563eb',
      phaseF2L2: '#3b82f6',
      phaseF2L3: '#60a5fa',
      phaseF2L4: '#93c5fd',
      phaseOLL: '#ca8a04',
      phasePLL: '#16a34a',
      cubeWhite: '#ffffff',
      cubeYellow: '#eab308',
      cubeGreen: '#16a34a',
      cubeBlue: '#2563eb',
      cubeRed: '#dc2626',
      cubeOrange: '#ea580c',
    },
  },
  nord: {
    name: 'Nord',
    colors: {
      bg: '#2e3440',
      bgSecondary: '#3b4252',
      main: '#88c0d0',
      sub: '#616e88',
      subAlt: '#434c5e',
      text: '#d8dee9',
      error: '#bf616a',
      accent: '#88c0d0',
      accentHover: '#81a1c1',
      phaseCross: '#b48ead',
      phaseF2L1: '#5e81ac',
      phaseF2L2: '#81a1c1',
      phaseF2L3: '#88c0d0',
      phaseF2L4: '#8fbcbb',
      phaseOLL: '#ebcb8b',
      phasePLL: '#a3be8c',
      cubeWhite: '#eceff4',
      cubeYellow: '#ebcb8b',
      cubeGreen: '#a3be8c',
      cubeBlue: '#5e81ac',
      cubeRed: '#bf616a',
      cubeOrange: '#d08770',
    },
  },
  dracula: {
    name: 'Dracula',
    colors: {
      bg: '#282a36',
      bgSecondary: '#44475a',
      main: '#bd93f9',
      sub: '#6272a4',
      subAlt: '#383a47',
      text: '#f8f8f2',
      error: '#ff5555',
      accent: '#bd93f9',
      accentHover: '#a87bff',
      phaseCross: '#ff79c6',
      phaseF2L1: '#6272a4',
      phaseF2L2: '#8be9fd',
      phaseF2L3: '#66d9ef',
      phaseF2L4: '#a4ffff',
      phaseOLL: '#f1fa8c',
      phasePLL: '#50fa7b',
      cubeWhite: '#f8f8f2',
      cubeYellow: '#f1fa8c',
      cubeGreen: '#50fa7b',
      cubeBlue: '#6272a4',
      cubeRed: '#ff5555',
      cubeOrange: '#ffb86c',
    },
  },
  monokai: {
    name: 'Monokai',
    colors: {
      bg: '#272822',
      bgSecondary: '#3e3d32',
      main: '#f92672',
      sub: '#75715e',
      subAlt: '#3e3d32',
      text: '#f8f8f2',
      error: '#f92672',
      accent: '#a6e22e',
      accentHover: '#98d91e',
      phaseCross: '#ae81ff',
      phaseF2L1: '#66d9ef',
      phaseF2L2: '#7fdfff',
      phaseF2L3: '#99e5ff',
      phaseF2L4: '#b3ecff',
      phaseOLL: '#e6db74',
      phasePLL: '#a6e22e',
      cubeWhite: '#f8f8f2',
      cubeYellow: '#e6db74',
      cubeGreen: '#a6e22e',
      cubeBlue: '#66d9ef',
      cubeRed: '#f92672',
      cubeOrange: '#fd971f',
    },
  },
  ocean: {
    name: 'Ocean',
    colors: {
      bg: '#1b2838',
      bgSecondary: '#1e3a5f',
      main: '#66c0f4',
      sub: '#4a6fa5',
      subAlt: '#2a475e',
      text: '#c7d5e0',
      error: '#e74c3c',
      accent: '#66c0f4',
      accentHover: '#52a8db',
      phaseCross: '#9b59b6',
      phaseF2L1: '#2980b9',
      phaseF2L2: '#3498db',
      phaseF2L3: '#5dade2',
      phaseF2L4: '#85c1e9',
      phaseOLL: '#f39c12',
      phasePLL: '#27ae60',
      cubeWhite: '#ecf0f1',
      cubeYellow: '#f1c40f',
      cubeGreen: '#2ecc71',
      cubeBlue: '#3498db',
      cubeRed: '#e74c3c',
      cubeOrange: '#e67e22',
    },
  },
  matrix: {
    name: 'Matrix',
    colors: {
      bg: '#0d0208',
      bgSecondary: '#1a1a1a',
      main: '#00ff41',
      sub: '#008f11',
      subAlt: '#0d1f0d',
      text: '#00ff41',
      error: '#ff0000',
      accent: '#00ff41',
      accentHover: '#00cc33',
      phaseCross: '#9900ff',
      phaseF2L1: '#0066ff',
      phaseF2L2: '#0088ff',
      phaseF2L3: '#00aaff',
      phaseF2L4: '#00ccff',
      phaseOLL: '#ffff00',
      phasePLL: '#00ff41',
      cubeWhite: '#00ff41',
      cubeYellow: '#ccff00',
      cubeGreen: '#00ff41',
      cubeBlue: '#0088ff',
      cubeRed: '#ff0000',
      cubeOrange: '#ff6600',
    },
  },
  midnight: {
    name: 'Midnight',
    colors: {
      bg: '#0f0f1a',
      bgSecondary: '#1a1a2e',
      main: '#7c3aed',
      sub: '#4b5563',
      subAlt: '#252538',
      text: '#e5e5e5',
      error: '#ef4444',
      accent: '#7c3aed',
      accentHover: '#6d28d9',
      phaseCross: '#a855f7',
      phaseF2L1: '#3b82f6',
      phaseF2L2: '#60a5fa',
      phaseF2L3: '#93c5fd',
      phaseF2L4: '#bfdbfe',
      phaseOLL: '#eab308',
      phasePLL: '#22c55e',
      cubeWhite: '#e5e5e5',
      cubeYellow: '#eab308',
      cubeGreen: '#22c55e',
      cubeBlue: '#3b82f6',
      cubeRed: '#ef4444',
      cubeOrange: '#f97316',
    },
  },
  everforest: {
    name: 'Everforest',
    colors: {
      bg: '#2d353b',
      bgSecondary: '#343f44',
      main: '#a7c080',
      sub: '#859289',
      subAlt: '#3d484d',
      text: '#d3c6aa',
      error: '#e67e80',
      accent: '#a7c080',
      accentHover: '#8fb573',
      phaseCross: '#d699b6',
      phaseF2L1: '#7fbbb3',
      phaseF2L2: '#83c092',
      phaseF2L3: '#a7c080',
      phaseF2L4: '#d3c6aa',
      phaseOLL: '#dbbc7f',
      phasePLL: '#a7c080',
      cubeWhite: '#d3c6aa',
      cubeYellow: '#dbbc7f',
      cubeGreen: '#a7c080',
      cubeBlue: '#7fbbb3',
      cubeRed: '#e67e80',
      cubeOrange: '#e69875',
    },
  },
  oneDark: {
    name: 'One Dark',
    colors: {
      bg: '#282c34',
      bgSecondary: '#21252b',
      main: '#61afef',
      sub: '#5c6370',
      subAlt: '#3e4451',
      text: '#abb2bf',
      error: '#e06c75',
      accent: '#61afef',
      accentHover: '#528bcc',
      phaseCross: '#c678dd',
      phaseF2L1: '#61afef',
      phaseF2L2: '#56b6c2',
      phaseF2L3: '#7ec8e3',
      phaseF2L4: '#a8d8ea',
      phaseOLL: '#e5c07b',
      phasePLL: '#98c379',
      cubeWhite: '#abb2bf',
      cubeYellow: '#e5c07b',
      cubeGreen: '#98c379',
      cubeBlue: '#61afef',
      cubeRed: '#e06c75',
      cubeOrange: '#d19a66',
    },
  },
  catppuccinMocha: {
    name: 'Catppuccin Mocha',
    colors: {
      bg: '#1e1e2e',
      bgSecondary: '#313244',
      main: '#cba6f7',
      sub: '#6c7086',
      subAlt: '#45475a',
      text: '#cdd6f4',
      error: '#f38ba8',
      accent: '#cba6f7',
      accentHover: '#b4befe',
      phaseCross: '#f5c2e7',
      phaseF2L1: '#89b4fa',
      phaseF2L2: '#74c7ec',
      phaseF2L3: '#89dceb',
      phaseF2L4: '#94e2d5',
      phaseOLL: '#f9e2af',
      phasePLL: '#a6e3a1',
      cubeWhite: '#cdd6f4',
      cubeYellow: '#f9e2af',
      cubeGreen: '#a6e3a1',
      cubeBlue: '#89b4fa',
      cubeRed: '#f38ba8',
      cubeOrange: '#fab387',
    },
  },
  catppuccinMacchiato: {
    name: 'Catppuccin Macchiato',
    colors: {
      bg: '#24273a',
      bgSecondary: '#363a4f',
      main: '#c6a0f6',
      sub: '#6e738d',
      subAlt: '#494d64',
      text: '#cad3f5',
      error: '#ed8796',
      accent: '#c6a0f6',
      accentHover: '#b7bdf8',
      phaseCross: '#f5bde6',
      phaseF2L1: '#8aadf4',
      phaseF2L2: '#7dc4e4',
      phaseF2L3: '#91d7e3',
      phaseF2L4: '#8bd5ca',
      phaseOLL: '#eed49f',
      phasePLL: '#a6da95',
      cubeWhite: '#cad3f5',
      cubeYellow: '#eed49f',
      cubeGreen: '#a6da95',
      cubeBlue: '#8aadf4',
      cubeRed: '#ed8796',
      cubeOrange: '#f5a97f',
    },
  },
  catppuccinFrappe: {
    name: 'Catppuccin Frappé',
    colors: {
      bg: '#303446',
      bgSecondary: '#414559',
      main: '#ca9ee6',
      sub: '#737994',
      subAlt: '#51576d',
      text: '#c6d0f5',
      error: '#e78284',
      accent: '#ca9ee6',
      accentHover: '#babbf1',
      phaseCross: '#f4b8e4',
      phaseF2L1: '#8caaee',
      phaseF2L2: '#85c1dc',
      phaseF2L3: '#99d1db',
      phaseF2L4: '#81c8be',
      phaseOLL: '#e5c890',
      phasePLL: '#a6d189',
      cubeWhite: '#c6d0f5',
      cubeYellow: '#e5c890',
      cubeGreen: '#a6d189',
      cubeBlue: '#8caaee',
      cubeRed: '#e78284',
      cubeOrange: '#ef9f76',
    },
  },
  catppuccinLatte: {
    name: 'Catppuccin Latte',
    colors: {
      bg: '#eff1f5',
      bgSecondary: '#e6e9ef',
      main: '#8839ef',
      sub: '#8c8fa1',
      subAlt: '#ccd0da',
      text: '#4c4f69',
      error: '#d20f39',
      accent: '#8839ef',
      accentHover: '#7287fd',
      phaseCross: '#ea76cb',
      phaseF2L1: '#1e66f5',
      phaseF2L2: '#04a5e5',
      phaseF2L3: '#209fb5',
      phaseF2L4: '#179299',
      phaseOLL: '#df8e1d',
      phasePLL: '#40a02b',
      cubeWhite: '#dce0e8',
      cubeYellow: '#df8e1d',
      cubeGreen: '#40a02b',
      cubeBlue: '#1e66f5',
      cubeRed: '#d20f39',
      cubeOrange: '#fe640b',
    },
  },
  rosePine: {
    name: 'Rosé Pine',
    colors: {
      bg: '#191724',
      bgSecondary: '#1f1d2e',
      main: '#ebbcba',
      sub: '#6e6a86',
      subAlt: '#26233a',
      text: '#e0def4',
      error: '#eb6f92',
      accent: '#ebbcba',
      accentHover: '#c4a7e7',
      phaseCross: '#c4a7e7',
      phaseF2L1: '#31748f',
      phaseF2L2: '#9ccfd8',
      phaseF2L3: '#ebbcba',
      phaseF2L4: '#f6c177',
      phaseOLL: '#f6c177',
      phasePLL: '#9ccfd8',
      cubeWhite: '#e0def4',
      cubeYellow: '#f6c177',
      cubeGreen: '#9ccfd8',
      cubeBlue: '#31748f',
      cubeRed: '#eb6f92',
      cubeOrange: '#ebbcba',
    },
  },
  rosePineMoon: {
    name: 'Rosé Pine Moon',
    colors: {
      bg: '#232136',
      bgSecondary: '#2a273f',
      main: '#ea9a97',
      sub: '#6e6a86',
      subAlt: '#393552',
      text: '#e0def4',
      error: '#eb6f92',
      accent: '#ea9a97',
      accentHover: '#c4a7e7',
      phaseCross: '#c4a7e7',
      phaseF2L1: '#3e8fb0',
      phaseF2L2: '#9ccfd8',
      phaseF2L3: '#ea9a97',
      phaseF2L4: '#f6c177',
      phaseOLL: '#f6c177',
      phasePLL: '#9ccfd8',
      cubeWhite: '#e0def4',
      cubeYellow: '#f6c177',
      cubeGreen: '#9ccfd8',
      cubeBlue: '#3e8fb0',
      cubeRed: '#eb6f92',
      cubeOrange: '#ea9a97',
    },
  },
  rosePineDawn: {
    name: 'Rosé Pine Dawn',
    colors: {
      bg: '#faf4ed',
      bgSecondary: '#fffaf3',
      main: '#d7827e',
      sub: '#9893a5',
      subAlt: '#f2e9e1',
      text: '#575279',
      error: '#b4637a',
      accent: '#d7827e',
      accentHover: '#907aa9',
      phaseCross: '#907aa9',
      phaseF2L1: '#286983',
      phaseF2L2: '#56949f',
      phaseF2L3: '#d7827e',
      phaseF2L4: '#ea9d34',
      phaseOLL: '#ea9d34',
      phasePLL: '#56949f',
      cubeWhite: '#f2e9e1',
      cubeYellow: '#ea9d34',
      cubeGreen: '#56949f',
      cubeBlue: '#286983',
      cubeRed: '#b4637a',
      cubeOrange: '#d7827e',
    },
  },
  githubDark: {
    name: 'GitHub Dark',
    colors: {
      bg: '#0d1117',
      bgSecondary: '#161b22',
      main: '#58a6ff',
      sub: '#8b949e',
      subAlt: '#21262d',
      text: '#c9d1d9',
      error: '#f85149',
      accent: '#58a6ff',
      accentHover: '#79c0ff',
      phaseCross: '#bc8cff',
      phaseF2L1: '#58a6ff',
      phaseF2L2: '#79c0ff',
      phaseF2L3: '#a5d6ff',
      phaseF2L4: '#56d4dd',
      phaseOLL: '#e3b341',
      phasePLL: '#3fb950',
      cubeWhite: '#c9d1d9',
      cubeYellow: '#e3b341',
      cubeGreen: '#3fb950',
      cubeBlue: '#58a6ff',
      cubeRed: '#f85149',
      cubeOrange: '#db6d28',
    },
  },
  githubLight: {
    name: 'GitHub Light',
    colors: {
      bg: '#ffffff',
      bgSecondary: '#f6f8fa',
      main: '#0969da',
      sub: '#57606a',
      subAlt: '#d0d7de',
      text: '#24292f',
      error: '#cf222e',
      accent: '#0969da',
      accentHover: '#0550ae',
      phaseCross: '#8250df',
      phaseF2L1: '#0969da',
      phaseF2L2: '#218bff',
      phaseF2L3: '#54aeff',
      phaseF2L4: '#80ccff',
      phaseOLL: '#9a6700',
      phasePLL: '#1a7f37',
      cubeWhite: '#f6f8fa',
      cubeYellow: '#9a6700',
      cubeGreen: '#1a7f37',
      cubeBlue: '#0969da',
      cubeRed: '#cf222e',
      cubeOrange: '#bc4c00',
    },
  },
  gruvboxDark: {
    name: 'Gruvbox Dark',
    colors: {
      bg: '#282828',
      bgSecondary: '#3c3836',
      main: '#fabd2f',
      sub: '#928374',
      subAlt: '#504945',
      text: '#ebdbb2',
      error: '#fb4934',
      accent: '#fabd2f',
      accentHover: '#d79921',
      phaseCross: '#d3869b',
      phaseF2L1: '#83a598',
      phaseF2L2: '#8ec07c',
      phaseF2L3: '#b8bb26',
      phaseF2L4: '#ebdbb2',
      phaseOLL: '#fabd2f',
      phasePLL: '#b8bb26',
      cubeWhite: '#ebdbb2',
      cubeYellow: '#fabd2f',
      cubeGreen: '#b8bb26',
      cubeBlue: '#83a598',
      cubeRed: '#fb4934',
      cubeOrange: '#fe8019',
    },
  },
  gruvboxLight: {
    name: 'Gruvbox Light',
    colors: {
      bg: '#fbf1c7',
      bgSecondary: '#f2e5bc',
      main: '#d79921',
      sub: '#7c6f64',
      subAlt: '#ebdbb2',
      text: '#3c3836',
      error: '#cc241d',
      accent: '#d79921',
      accentHover: '#b57614',
      phaseCross: '#b16286',
      phaseF2L1: '#458588',
      phaseF2L2: '#689d6a',
      phaseF2L3: '#98971a',
      phaseF2L4: '#504945',
      phaseOLL: '#d79921',
      phasePLL: '#98971a',
      cubeWhite: '#fbf1c7',
      cubeYellow: '#d79921',
      cubeGreen: '#98971a',
      cubeBlue: '#458588',
      cubeRed: '#cc241d',
      cubeOrange: '#d65d0e',
    },
  },
  tokyoNight: {
    name: 'Tokyo Night',
    colors: {
      bg: '#1a1b26',
      bgSecondary: '#24283b',
      main: '#7aa2f7',
      sub: '#565f89',
      subAlt: '#414868',
      text: '#c0caf5',
      error: '#f7768e',
      accent: '#7aa2f7',
      accentHover: '#7dcfff',
      phaseCross: '#bb9af7',
      phaseF2L1: '#7aa2f7',
      phaseF2L2: '#7dcfff',
      phaseF2L3: '#2ac3de',
      phaseF2L4: '#73daca',
      phaseOLL: '#e0af68',
      phasePLL: '#9ece6a',
      cubeWhite: '#c0caf5',
      cubeYellow: '#e0af68',
      cubeGreen: '#9ece6a',
      cubeBlue: '#7aa2f7',
      cubeRed: '#f7768e',
      cubeOrange: '#ff9e64',
    },
  },
  solarizedDark: {
    name: 'Solarized Dark',
    colors: {
      bg: '#002b36',
      bgSecondary: '#073642',
      main: '#268bd2',
      sub: '#586e75',
      subAlt: '#073642',
      text: '#839496',
      error: '#dc322f',
      accent: '#268bd2',
      accentHover: '#2aa198',
      phaseCross: '#6c71c4',
      phaseF2L1: '#268bd2',
      phaseF2L2: '#2aa198',
      phaseF2L3: '#93a1a1',
      phaseF2L4: '#859900',
      phaseOLL: '#b58900',
      phasePLL: '#859900',
      cubeWhite: '#eee8d5',
      cubeYellow: '#b58900',
      cubeGreen: '#859900',
      cubeBlue: '#268bd2',
      cubeRed: '#dc322f',
      cubeOrange: '#cb4b16',
    },
  },
  solarizedLight: {
    name: 'Solarized Light',
    colors: {
      bg: '#fdf6e3',
      bgSecondary: '#eee8d5',
      main: '#268bd2',
      sub: '#93a1a1',
      subAlt: '#eee8d5',
      text: '#657b83',
      error: '#dc322f',
      accent: '#268bd2',
      accentHover: '#2aa198',
      phaseCross: '#6c71c4',
      phaseF2L1: '#268bd2',
      phaseF2L2: '#2aa198',
      phaseF2L3: '#586e75',
      phaseF2L4: '#859900',
      phaseOLL: '#b58900',
      phasePLL: '#859900',
      cubeWhite: '#fdf6e3',
      cubeYellow: '#b58900',
      cubeGreen: '#859900',
      cubeBlue: '#268bd2',
      cubeRed: '#dc322f',
      cubeOrange: '#cb4b16',
    },
  },
}

export const STANDARD_CUBE_COLORS = {
  cubeWhite: '#ffffff',
  cubeYellow: '#ffd500',
  cubeGreen: '#009b48',
  cubeBlue: '#0045ad',
  cubeRed: '#b90000',
  cubeOrange: '#ff5900',
}

export type CubeTheme = 'current' | 'standard' | keyof typeof themes

export function getCubeColors(cubeTheme: CubeTheme, currentTheme: string): Record<string, string> {
  if (cubeTheme === 'standard') {
    return STANDARD_CUBE_COLORS
  }

  const themeToUse = cubeTheme === 'current' ? currentTheme : cubeTheme
  const theme = themes[themeToUse] || themes.kitsune

  return {
    cubeWhite: theme.colors.cubeWhite,
    cubeYellow: theme.colors.cubeYellow,
    cubeGreen: theme.colors.cubeGreen,
    cubeBlue: theme.colors.cubeBlue,
    cubeRed: theme.colors.cubeRed,
    cubeOrange: theme.colors.cubeOrange,
  }
}

export const themeKeys = Object.keys(themes) as (keyof typeof themes)[]

export function applyTheme(themeKey: string): void {
  // In React Native, themes are applied directly in components
  // CSS variables don't exist, so this is a no-op
  // The theme colors are accessed via getCubeColors() and other getter functions
  const theme = themes[themeKey] || themes.kitsune
  // No-op for React Native - themes are handled via context/props
}
